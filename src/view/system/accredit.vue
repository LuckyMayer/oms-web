      ...
      <div class="row form-horizontal">
        <div class="col-md-12 bgFFF" style="border-bottom: 3px solid #f2f2f2;">
          <div class="form-group">
            <label class="control-label" style="font-weight: bold;float:left;margin-left:20px">授权权限:
              <font color="red">{{userMessage.userName}}</font>
            </label>
            <a class="col-sm-1 control-label">取消</a>
            <div class="col-sm-1 textAlignRight">
              <button class="btn btn-sm btn-primary" @click="userAccredit">确认</button>
            </div>
          </div>
        </div>
        <div class="col-md-12 bgFFF">
          <div class="form-group">
            <label class="control-label col-sm-1" style="margin-right: 20px;">可授权业务组</label>
            <div class="col-sm-2">
          ...
          <div class="form-group">
            <label class="control-label col-sm-1" style="margin-right: 20px;">授权权限</label>
          </div>
        </div>
    ...
    <div class="popup-wrap-yjlu popup-assign" v-if="isOpen">
      <div class="popup-content row form-inline">
        ...
        <div class="popup-footer">
          <button class="btn btn-sm btn-primary" @click="isOpen = false;"><i class="fa fa-check"></i>确定</button>
...
export default {
  ...
  data () {
    return {
      isOpen: false,
      accreditData: {
        privilegeUserId: this.$route.params.id,
      ...
      http({
        ...
        success (res) {
          if (res.success) {
            ...
            res.data.adminResources.forEach(function (item){
...
////角色处理--end
  userAccredit () {
      ...
      _self.accreditData.addPartyIdList=party_change.insert;
      _self.accreditData.deletePartyIdList=party_change.delete;
      _self.accreditData.addRoleIdList=role_change.insert;
      _self.accreditData.deleteRoleIdList=role_change.delete;
      _self.accreditData.addResourceIdList=resource_change.insert;
      _self.accreditData.deleteResourceIdList=resource_change.delete;
      http({
        instance: _self,
        url: api.userAccredit,
        type: 'post',
        hasLoading: true,
        data: {
          data: _self.accreditData
        },
        success (res) {
          if (res.success) {
            layer.msg('授权权限成功', {time: 2000});
          }
        }
      });
    },
